...
3.  The output document has a table referenced in the text.
    What are the two components necessary for creating the reference?

4.  Can you find the R output of the analysis in one of the paragraphs in the PDF?
    There are two ways in which code is integrated into `analysis.qmd`. What are they, and how do they differ?

5.  At the beginning of `analysis.qmd` is a section fenced by `---`.
...
```{=typst}
6.  There are two blocks fenced by ```` ```{r} ```` and ```` ``` ```` in `analysis.qmd`, but only one of them appears in the output.
    How is this controlled?
```

7.  **Bonus: Did you uncover the error in the analysis?**
